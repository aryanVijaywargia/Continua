...
These surfaces are planned, not implemented. See [Roadmap & status](/roadmap) for the full
breakdown.

- **Live WebSocket runtime**: trace detail is polling-based today.
- **Proxy capture runtime**.
- **Trace-replay execution**: re-running recorded traces against new code or model versions.
  This is distinct from engine crash-recovery workflow replay, which is implemented.
- **Feature-complete TypeScript SDK**: the package has build scaffolding and a placeholder client class whose only member is a `baseUrl` getter.
