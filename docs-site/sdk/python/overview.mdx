...
- [`EngineControlClient`](/sdk/python/engine-control) — control plane for the durable
  engine surface (scaffolded)

### Decorators and context managers

- [`@trace`](/sdk/python/tracing) — wrap a top-level agent function
- [`@span`](/sdk/python/tracing) — open a span context manager
- [`@session`](/sdk/python/tracing) — group traces under a session

### Context classes

- `TraceContext`, `SpanContext`, `SessionContext`, `ActivityTaskContext`,
  [`ActivityWorker`](/sdk/python/activity-worker)
...
| `continua.span` | `@span` context manager + `SpanContext` and its 19 event methods |
| `continua.session` | `@session` context manager + `SessionContext` |
| `continua.worker` | `ActivityWorker`, `@activity` decorator (engine surface) |
| `continua.engine_control` | `EngineControlClient` (engine surface) |
| `continua.exceptions` | All 8 exception classes |
...
<CardGroup cols={2}>
  ...
  <Card title="Tracing" icon="route" href="/sdk/python/tracing">
    `@trace`, `@span`, `@session` — decorators and context managers in depth.
  </Card>
